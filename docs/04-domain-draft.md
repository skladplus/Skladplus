...
| `Shipment` | orderId `@unique` (#11) · ttn? · npStatus? (#59) · codInTransitKop? (#53) · npCostKop? · npCostQuotedAt? (момент успешного ответа API, не запроса) · handedOverAt? · carrierScanAt? · deliveredAt? | двухстадийный финиш SLA (#4); `npCostKop` — котировка API НП на момент создания ТТН, справочная: счёт выставляет НП клиенту напрямую (#12) |
