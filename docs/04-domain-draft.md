...
| #26 | повторная отправка — новый `Order` со ссылкой: поле `reshipmentOfId?` заводится (опросник 2026-08-29) |
| #28 | блокировок при OVERDUE нет (опросник 2026-08-29) — флаги на `Account` не заводятся |
| #33 | списание проводит менеджер с фото (опросник 2026-08-29) — отдельный статус подтверждения не нужен |
