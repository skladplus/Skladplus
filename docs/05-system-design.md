...
│   └── migrations/            ✓
└── src/
    ├── scope.ts               ✓ # forAccount → AccountScope; риск №1 здесь
...
через который запрос делается. Гарантией это станет, когда CI запретит в `apps/`
прямой Prisma и `unsafePrisma()`; пока обход открыт — принятый риск
(06-architecture). `StaffScope` — явное ослабление для экранов склада.
...
Восстановление данных — PITR Neon; целевое окно 7 дней, это предел тарифа
Launch, фактическое значение — в настройках проекта. Репетиция
