...
задачам 08. Принцип из 03 §2.1: «один модуль = одна папка роута + одна запись
реестра».
...
| 2 | **#44 Auth-провайдер** `[contract]`, задача 1.4 | **better-auth** — уже рекомендован в 02 §8: серверные сессии в Postgres, argon2id, схема БД своя (блок A замещается его таблицами) | Auth.js v5 — зрелее экосистема, но сессии и модель под нашу спеку придётся гнуть сильнее |
...
| 4 | **Лимит Excel-импорта** — оставить 10 МБ (тогда загрузка только через хранилище из №3) или снизить до 4 МБ в спеке 02 §4.2.5? | **Оставить 10 МБ через хранилище** — каталог клиента с фото-ссылками легко переваливает за 4 МБ | снизить лимит — быстрее, но упрётся в первый же большой каталог |
