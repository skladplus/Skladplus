...
по кругу. Цифры нагрузки — §1, узкие места — §7; ответы опросника
по атрибутам — #82–85.
...
|---|---|---|---|
| 1 | **#36 Воркер** — чем гонять снимки остатков (#2), переопрос НП (#53), просрочку счетов, дайджесты (#51)? `[contract]`, этап 3 | **Vercel Cron → `/api/cron/*`** + таблица прогонов джобов в Postgres (идемпотентность, ручной перезапуск). Ноль новой инфраструктуры; всё в §1 укладывается в лимиты. Опросник 2026-08-29: alex — за; финализация со Step, issue #68 | отдельный процесс-воркер (нужен хостинг вне Vercel); очередь Redis/QStash — отвергнутый Redis вернётся, потребителя всё ещё нет |

Остальные вопросы закрыты опросником 2026-08-29: хранилище — Vercel Blob (#38),
