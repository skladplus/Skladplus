...
- `packages/db` — единственная дверь к данным: `AccountScope.forAccount()`
  ...
  персонала (06-architecture). Прямой Prisma из `apps/` запрещён.
...
экранов склада (06-architecture). Это не абстракция ради тестов, а несущая
...
Redis отвергнут ранее (06-architecture): при чтениях 50:1 узким местом
