...
устроена целиком»; решения из §8 ждут ответа людей.
...
flowchart TB
  ...
  ST -- "браузер: сборка, приёмка,<br/>инвентаризация" --> SYS
...
```mermaid
flowchart TB
  B["Браузер<br/>[клиентское устройство]"]
...
- Каркас UI (03 §2–3) — реестры модулей/виджетов/настроек и библиотека
  паттернов («раздел-таблица», «двухколоночная форма»…).
...
│   ├── print/                   # print-роуты без layout: акты (#15); этикетка снята (#84)
│   ├── api/
...
по кругу. Цифры нагрузки — §1, узкие места — §7; решения
по атрибутам — #82–85.
...
Цели восстановления: RPO — меньше 10 минут (PITR пишется по WAL), RTO —
два часа внутри рабочего окна. Репетиция (#41) проверяет обе: восстановление
на точку не старше 10 минут до сбоя, подъём за два часа.
...
возможностей стека; нагрузочное тестирование появляется при росте цифр §1
на порядок (§5). Бюджет отклика: p95 серверного ответа интерактивных
экранов и действий под таймером SLA («Взяти в роботу», «Зібрано») —
полсекунды; полная отрисовка складского экрана — секунда.

Тяжёлое в бюджет не входит: отчёты, импорт и экспорт уходят в предпросмотр
и воркер (§7 №6, №8). Пока наблюдаемость не приехала (Sentry и uptime-чек —
вместе с воркером, #82), бюджет проверяется метриками Vercel руками.
...
обрезка потребовала бы замыкающих снимков. Фото и обезличивание ПДн
получателей (#18) — шесть месяцев, исполняет воркер (#83).

Бессрочность законна, потому что ПДн в журналах не живут: данные
получателей — только в `Recipient`, обезличивание не рвёт ссылки (#18);
журналы несут идентификаторы (`staffId`, `createdBy`) и операционные
комментарии — ПДн в комментарий не пишутся, за этим следят подсказка
поля и ревью экранов (#83).
...
интервалы поллинга (#87). Только деплоем меняются правила: машины
...
или интеграция контейнеры не трогают; если контейнер приходится менять —
точка расширения сломана, и чинится она, а не обходится копией экрана.
...
как DoD (03 §3). Диагностика инцидентов — Sentry и uptime-чек (#82).

**Удобство использования — мера продуктовая: SLA.** Складские экраны
проектируются под «30 рабочих минут» (02 §6) и desktop-first: сканера
и печати этикеток нет — этикетку печатает отделение НП по созданной
системой ТТН (#84). Недоступность НП сборку не держит: заказ завершается
без ТТН, она добирается кнопкой после восстановления (#85). Язык —
украинский, единственный; a11y — минимум 03 §3, уровень WCAG не берём.
...
| 6 | **Excel-импорт: 02 §4.2.5 обещает файлы до 10 МБ, а лимит тела serverless-запроса Vercel — 4,5 МБ** | на первом большом каталоге | решено (#86): лимит 10 МБ остаётся, файл уходит по подписанному URL напрямую в Vercel Blob (#38), обработка — из воркера |
...
## 8. Решения, которые нужны людям

Формат: рекомендация — первой, решение за вами (для `[contract]` —
за обоими).

| # | Вопрос | Рекомендация | Альтернативы |
|---|---|---|---|
| 1 | **#36 Воркер** — чем гонять снимки остатков (#2), переопрос НП (#53), просрочку счетов, дайджесты (#51)? `[contract]`, этап 3 | **Vercel Cron → `/api/cron/*`** + таблица прогонов джобов в Postgres (идемпотентность, ручной перезапуск). Ноль новой инфраструктуры; всё в §1 укладывается в лимиты. Позиция alex — за; финализация со Step, issue #68 | отдельный процесс-воркер (нужен хостинг вне Vercel); очередь Redis/QStash — отвергнутый Redis вернётся, потребителя всё ещё нет |

Остальные вопросы закрыты: хранилище — Vercel Blob (#38),
лимит Excel — 10 МБ через хранилище (#86), поллинг — 10/5/30 с (#87).
После ответа №1 строка-решение добавляется в `docs/06-architecture.md`
тем же PR, что реализует воркер.
